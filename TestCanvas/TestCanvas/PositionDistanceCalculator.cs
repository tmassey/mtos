...
namespace TestCanvas
{
    ...
    public class PositionDistanceCalculator : IPositionDistanceCalculator
    {
        ...
        public double CalculateDistance(IPosition pos1, IPosition pos2, DistanceUnit unit)
        {
            ...
            double dLat = _angleConverter.ConvertDegreesToRadians(pos2.Latitude) -
                          _angleConverter.ConvertDegreesToRadians(pos1.Latitude);
            ...
            double a = Math.Sin(dLat/2)*Math.Sin(dLat/2) +
                       Math.Cos(_angleConverter.ConvertDegreesToRadians(pos1.Latitude))*
                       Math.Cos(_angleConverter.ConvertDegreesToRadians(pos2.Latitude))*Math.Sin(dLon/2)*
                       Math.Sin(dLon/2);
        ...
        public double CalculateRhumbDistance(IPosition pos1, IPosition pos2, DistanceUnit unit)
        {
            ...
            double dLat = _angleConverter.ConvertDegreesToRadians(pos2.Latitude - pos1.Latitude);
            double dLon = _angleConverter.ConvertDegreesToRadians(Math.Abs(pos2.Longitude - pos1.Longitude));
